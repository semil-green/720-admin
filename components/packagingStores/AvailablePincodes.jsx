...
} from "@/store/slices/packaging-center/packaging-center.slice";
...
export default function AvailablePincodes({
    ...
    const fetchAllPincodesOfId = async (
        ...
        try {
            ...
            if (fetchData?.status === 200) {
                ...
                dispatch(setPackagingCenterPaginatedPincodeData(fetchData?.data?.data));
            }
        } catch (err) {
    ...
    const paginatedPackagingCenters = useSelector(
        (state) => state.packagingStoreSlice.packagingCenterPaginatedPincodes
    );
    ...
    const handleSubmit = async (e) => {
        ...
        if (!pincode.trim() || !deliveryCharge.trim()) {
            toast.error("Missing Fields", {
                description: "Please enter both Pincode and Delivery Charge",
            });
            return;
        }
        ...
        if (res?.status === 200) {
            ...
            dispatch(addPackagingCenterPaginatedPincodeData(newPincode))
    ...
    const handleUpdatePinCode = async (e) => {
        ...
        if (res?.status === 200) {
            ...
            dispatch(updatePackagingCenterPaginatedPincodeData(updatedPincode))
            setIsEdit(false);
            setPincode("");
            setDeliveryCharge("");
            setPincodeId("");
    ...
    return (
        <div className="flex flex-col gap-2 min-w-[280px]">
            ...
            <table className="min-w-full table-auto border border-gray-300 rounded-md mt-2">
                ...
                <tbody>
                    {paginatedPackagingCenters?.map((pincodeItem, index) => (
                    ...
                    {paginatedPackagingCenters?.length === 0 && (
                        <tr>
