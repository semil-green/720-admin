...
const SalesOverTimeChart = ({ chartData, startDate, endDate }) => {
    const labels = chartData?.data?.map((item) => item?.order_date);
    const values = chartData?.data?.map((item) => Number(item?.total_amount));
    ...
    const exportToExcel = () => {
        ...
        dataToExport.push({
            "Total Tax": chartData?.summary?.total_tax,
            "Total Sales": chartData?.summary?.total_amount,
        });
        ...
        XLSX.writeFile(wb, "sales-over-time.xlsx");
    };

    return (
        <>
            <div className="my-2">
                <h1 className="font-md text-lg underline underline-offset-4 ">Total Sales :</h1>
                <p className="my-2 font-bold text-xl">₹ {chartData?.summary?.total_amount}</p>
            </div>
