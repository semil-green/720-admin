...
const TotalOrdersChart = ({ chartData, startDate, endDate }) => {
    const labels = chartData?.data?.map((item) => item?.order_date);
    const values = chartData?.data?.map((item) => Number(item?.total_orders));
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                accessorKey: "avg_quantity_per_order",
            },
            {
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, "total-orders-over-time.xlsx");
    };
    ...
    return (
        <>
            ...
            <div className="my-2">
                <h1 className="font-md text-lg underline underline-offset-4 ">Orders :</h1>
                <p className="my-2 font-bold text-xl">{chartData?.summary?.total_orders}</p>
            </div>
